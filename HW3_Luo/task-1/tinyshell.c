      ...
      if (C.redirection == 1)
      {
        ...
        printf("DEBUG: Redirection enabled\n");
         ...
         // printf("%s",C.ofile);
          ...
          int openfile = open(C.ofile, O_CREAT|O_WRONLY|O_TRUNC, S_IRWXU);
...
//        waitpid(fc, &status, 0);
      int status;
      if (waitpid(fc, &status, WUNTRACED | WCONTINUED) == -1)
